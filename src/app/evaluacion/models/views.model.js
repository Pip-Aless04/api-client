...
export class ViewsModel{
    ...
    static async getAll(filters) {
        try {
            ...
            // Asegúrate de que typeOfUser esté en filters o elimínalo si no es necesario
            if (filters.typeOfUser) {
                query += ` AND LOWER(C.col_personal_cargo) LIKE LOWER(@typeOfUser)`;
                params.push({ name: 'typeOfUser', type: sql.NVarChar, value: `%${filters.typeOfUser}%` });
            }
